...
def check_approval_status(request_id: str) -> Optional[str]:
    ...
    try:
        ...
        # Check for APPROVED or REJECTED anywhere in content (case-insensitive)
...
def wait_for_approval(
    ...
    if RICH_AVAILABLE:
        console.print()
        console.print(Panel(
            f"[bold yellow]⏳ Waiting for human approval...[/bold yellow]\n"
            f"[cyan]Request ID:[/cyan] [white]{request_id}[/white]\n"
            f"[cyan]Timeout:[/cyan] [white]{timeout_seconds}s[/white]\n"
            f"[cyan]Poll interval:[/cyan] [white]{poll_interval}s[/white]",
            border_style="yellow",
            padding=(0, 2)
        ))
        console.print()
    ...
    if RICH_AVAILABLE:
        with Progress(
            ...
            while True:
                ...
                # Update progress
                progress.update(task, completed=elapsed, status=f"Attempt {attempt}")

                # Check timeout
                if elapsed >= timeout_seconds:
                    progress.update(task, description="[red]⏱️  Timed out!")
                ...
                if status == "APPROVED":
                    progress.update(task, description="[green]✓ Approved!", completed=timeout_seconds)
                    log_action(f"Request approved: {request_id}", "SUCCESS")
                    move_to_done(request_id, "APPROVED")
                    return True
                elif status == "REJECTED":
                    progress.update(task, description="[red]✗ Rejected!", completed=timeout_seconds)
                    log_action(f"Request rejected: {request_id}", "INFO")
                    move_to_done(request_id, "REJECTED")
                    return False
                elif status is None:
                    progress.update(task, description="[red]✗ File not found!")
...
def main():
    ...
    if RICH_AVAILABLE:
        ...
        console.print(Panel.fit(
            "[bold green]👤 HUMAN APPROVAL AGENT[/bold green]\n"
    ...
    if args.details:
        try:
            ...
            if RICH_AVAILABLE:
                console.print("[bold red]✗ Invalid JSON in --details[/bold red]")
            else:
    ...
    try:
        ...
        if RICH_AVAILABLE:
            ...
            if approved:
                ...
                console.print(Panel(
                    "[bold red]✗ Request REJECTED[/bold red]\n"
                    "[dim]The action has been rejected by human reviewer[/dim]",
                    border_style="red",
                    padding=(1, 2)
                ))
            console.print()
        ...
        if RICH_AVAILABLE:
            console.print()
            console.print(Panel(
                f"[bold red]⏱️  Request TIMED OUT[/bold red]\n"
                f"[yellow]Timeout:[/yellow] [white]{e.timeout_seconds} seconds[/white]\n"
                f"[dim]No response received within the timeout period[/dim]",
                border_style="red",
                padding=(1, 2)
            ))
            console.print()
        ...
        if RICH_AVAILABLE:
            ...
            console.print(Panel(
                ...
                "[dim]Approval request cancelled[/dim]",
                border_style="yellow",
                padding=(1, 2)
            ))
            console.print()
